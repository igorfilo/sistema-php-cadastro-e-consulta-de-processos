...
if (linkExclusao != null) { 
	for ( var i = 0; i < linkExclusao.length; i++ ) {
		(function(i){
			var id_protocolo = linkExclusao[i].getAttribute('rel');

			if (linkExclusao[i].addEventListener){
		    	linkExclusao[i].addEventListener("click", function(){confirmaExclusao(id_protocolo);});
			}else if (linkExclusao[i].attachEvent) { 
				linkExclusao[i].attachEvent("onclick", function(){confirmaExclusao(id_protocolo);});
			}
		})(i);
	}
}
...
function confirmaExclusao(id){
	...
	if (retorno){
	    ...
	    formulario.action = "acao_protocolo.php";
